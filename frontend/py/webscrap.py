...
def ScrappingSnippet():
    ...
    for a in bs.find_all('a', attrs={"class": "p-reflist__wrap"}, href=True):
        results.append(a['href'].replace("./","https://digimon.net/reference/"))
...
def ScrappingEachOne():
    ...
    for link in results:
        ...
        errPrevent(dName)
        dCover = bs.find("li", attrs={"class": "p-ref__picitem"}).findChild()['src'].replace("..","https://digimon.net")
        errPrevent(dCover)
        ...
        name = dName
        digimonsDict[name] = {'cover': dCover, 'level': dLevel, 'type': dType, 'attribute': dAttr, 'move': dMove, 'description': dDesc}
